...
{
    // 1. Decompress the input dictionary if needed
    if (embed_tokens.shape(1) == 384) {
        auto scales = loader.get("model.embed_tokens.scales");
        auto zero_biases = mx::zeros(scales.shape(), scales.dtype()); 
        
        embed_tokens = mx::dequantize(embed_tokens, scales, zero_biases, 64, 4);
        std::cout << "[SYSTEM] Decompressed Token Embeddings to 3072 dimensions.\n";
    }

    // 2. Decompress the final output vocabulary (LM Head) if needed
    if (lm_head.shape(1) == 384) {
        auto scales = loader.get("lm_head.scales");
        auto zero_biases = mx::zeros(scales.shape(), scales.dtype()); 
        
        lm_head = mx::dequantize(lm_head, scales, zero_biases, 64, 4);
...
mx::array Phi3Model::forward(mx::array inputs, KVCache& cache) {
    ...
    // Note: The Language Model Head is usually kept unquantized for accuracy, 
    // so leave it as a standard matmul
